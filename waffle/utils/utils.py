...
# amino acid three to one letter mapping
THREE_TO_ONE = {
...
# helper: extract atmseq from pdb file using SeqIO
# NOTE: this will represent any discontinuous residue indices in the sequence as 'X'
def extract_atmseq_from_pdb_seqio(pdb_file: Path) -> Dict[str, str]:
    """
    Extract atmseq from pdb file.
    NOTE: this will represent any discontinuous residue indices in the sequence as 'X'
    """
    seqs = {}
...
# helper: extract atmseq from pdb file traversing residues
# NOTE: this will ignore discontinuous residues and represent any noncanonical amino acids as 'X'
def extract_atmseq_from_pdb_from_reses(pdb_path: Path, model_index: Optional[int]=None) -> Dict[str, str]:
    """
    Extract atmseq from pdb file.
    NOTE: this will ignore discontinuous residues and represent any noncanonical amino acids as 'X'

    If model_index is None, will consider all models in the structure file.
    it's only relevant if the structure contains more than one model, though commonly
    only one model is included.
    """
    seqs = {}
    parser = PDBParser(QUIET=True)
    structure = parser.get_structure(id=pdb_path.stem, file=pdb_path)
    if model_index is not None:
        structure = structure[model_index]
    for chain in unfold_entities(structure, 'C'):
        reses = chain.get_residues()
        seqs[chain.id] = ''.join([THREE_TO_ONE.get(aa3.get_resname(), 'X') for aa3 in reses])
    return seqs
...
# helper: determine if the file has SEQRES record
def has_seqres_record(struct_path: Path) -> bool:
...
def extract_seqres_from_pdb(struct_path: Path) -> Dict[str, str]:
    ...
    This function will use SEQRES if it exists, otherwise it will use the
    sequence in the ATOM records.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if has_seqres_record(pdb_fp):
            ...
            # use ATOM records
            logger.debug(f"No SEQRES record found in {pdb_fp.name}, using ATOM records instead")
            seqs = extract_atmseq_from_pdb_from_reses(pdb_path=pdb_fp)
        return seqs
